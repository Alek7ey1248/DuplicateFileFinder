...
//            if (contentHash == -10) continue;  // в методе calculateContentHash -10 если невалидный файл
...
//    }
    ...
    public List<List<File>> findDuplicates(List<File> fileList) {
        Map<Integer, List<File>> contentHashGroups = fileList.parallelStream()
                //.filter(file -> comparingTwoFiles.calculateContentHash(file) != -10)
                .collect(Collectors.groupingByConcurrent(comparingTwoFiles::calculateContentHash));

        contentHashGroups.remove(-10); // в методе calculateContentHash -10 если невалидный файл
